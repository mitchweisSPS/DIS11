...
connection = sqlite3.connect('/House_points/House_points.db', check_same_thread=False)
...
def studentregister():
    if request.method == 'POST':
        ...
        cur.execute("INSERT INTO students (student_id, student_firstname, student_lastname, student_email, student_password, student_year, house_name, tutor_group, student_points) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (student_id, student_firstname, student_lastname, student_email, student_password, student_year, house_name, tutor_group, "0"))
        connection.commit()
        connection.close()
    return render_template('register.html')
